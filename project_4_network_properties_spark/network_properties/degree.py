...
def simple(g):
	...
	# YOUR CODE HERE
    edges = g.edges
    flipped_edges = edges.withColumn('temp', edges.dst)
    flipped_edges = flipped_edges.withColumn('dst', flipped_edges.src)
    flipped_edges = flipped_edges.withColumn('src', flipped_edges.temp)
    flipped_edges = flipped_edges.drop('temp')
    ...
    edges = g.edges.unionAll(flipped_edges)
...
def degreedist(g):
    ...
    # YOUR CODE HERE
    degree_distribution = g.inDegrees.selectExpr('id as id', 'inDegree as degree').groupBy('degree').count()
