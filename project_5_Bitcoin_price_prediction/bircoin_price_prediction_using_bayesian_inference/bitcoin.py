...
def similarity(x, y):
    deviation_x = x.subtract(x.mean())
    deviation_y = y.subtract(y.mean())
    similarity = (deviation_x * deviation_y).sum() / (len(x) * x.std() * y.std())
    return similarity
...
def computeDelta(wt, X, Xi):
    ...
    Xi_new = Xi.drop('Yi', axis = 1)
    Yi_new = Xi['Yi']
    X_new = X[0 : len(X) - 1]
    Y_new = X[-1]

    numerator = (Xi_new.apply(lambda row: np.exp(wt * (similarity(row, X_new))), axis = 1) * Yi_new).sum()
    denominator = Xi_new.apply(lambda row: np.exp(wt * (similarity(row, X_new))), axis = 1).sum()
